-- Add user_id to sequences table for future use
-- This ensures sequences are user-specific when you start saving them

-- Step 1: Add user_id column
ALTER TABLE sequences ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id);

-- Step 2: Since table is empty, we can make it NOT NULL immediately
...
-- Step 4: Add index for campaign_id lookups (if sequences are campaign-specific)
