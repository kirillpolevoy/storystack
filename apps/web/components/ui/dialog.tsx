...
>(({ className, style, ...props }, ref) => {
  const overlayRef = React.useCallback((node: HTMLDivElement | null) => {
    ...
    if (node) {
      // Force the background color using !important via direct style manipulation
      ...
      console.log('[DialogOverlay] Rendered, node:', node)
      const computed = window.getComputedStyle(node)
      console.log('[DialogOverlay] backgroundColor:', computed.backgroundColor)
      console.log('[DialogOverlay] backdropFilter:', computed.backdropFilter)
      console.log('[DialogOverlay] zIndex:', computed.zIndex)
    }
  }, [ref])
  ...
  return (
    <DialogPrimitive.Overlay
      ref={overlayRef}
      className={cn(
        "fixed inset-0 z-[55] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        className
      )}
      style={{
