...
export async function extractLocationFromEXIF(
  ...
  try {
    ...
    if (latitude === null && exif && (exif as any).GPSLatitude && (exif as any).GPSLongitude) {
      ...
      // Try multiple possible key names for GPS reference
      const latRef = (exif as any).GPSLatitudeRef || (exif as any).GPSLatitudeRef || (exif as any).LatitudeRef;
      const lonRef = (exif as any).GPSLongitudeRef || (exif as any).GPSLongitudeRef || (exif as any).LongitudeRef;
      ...
      console.log('[extractLocationFromEXIF] GPS format data:', { 
        ...
        allExifKeys: Object.keys(exif).filter(k => k.toLowerCase().includes('gps') || k.toLowerCase().includes('lat') || k.toLowerCase().includes('lon'))
      });
    ...
    if (cityName) {
      ...
      if (checkResponse.ok) {
        ...
        if (resultCountry && ['cn', 'in', 'pk', 'bd', 'lk'].includes(resultCountry.toLowerCase()) && 
            finalLat > 25 && finalLat < 50 && finalLon > 80) {
  ...
  try {
    ...
    if (data.display_name && address.county) {
      ...
      for (const part of parts) {
        if (part.toLowerCase() !== address.county.toLowerCase() && 
